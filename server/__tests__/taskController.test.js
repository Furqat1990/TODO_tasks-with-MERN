const { getTasks, setTasks, updateTasks } = require('../controllers/taskController');
...
test('should set a new task for a user', async () => {
    ...
    await setTasks(req, res);// Ensure that the response contains the new task
...
test('should return a 401 error if user is not authorized to update the task', async () => {
    ...
    await expect(updateTasks(req, res)).rejects.toThrow('User is not authorized to update');
    expect(res.status).toHaveBeenCalledWith(401);
});